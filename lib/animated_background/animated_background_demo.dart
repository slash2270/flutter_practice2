...
import 'animated_fade_route.dart';
import 'animated_space_splash.dart';
import 'dart:math' as math;

class AnimatedBackgroundDemo extends StatefulWidget {
  const AnimatedBackgroundDemo({Key? key}) : super(key: key);

  @override
  _AnimatedBackgroundDemoState createState() => _AnimatedBackgroundDemoState();
}

class _AnimatedBackgroundDemoState extends State<AnimatedBackgroundDemo> with TickerProviderStateMixin {
  static const numBehaviours = 6;

  // Particles
  ParticleType? _particleType = ParticleType.Image;
  Image _image = Image.asset('assets/star_stroke.png');
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            Text('當前: ${_behaviourIndex + 1}超過$numBehaviours'),
          ],
        ),
        const SizedBox(height: 10.0),
        Text('行為: ${_behaviour.runtimeType.toString()}'),
        const SizedBox(height: 10.0), ..._behaviour is ParticleBehaviour
            ? _buildParticleSettings()
            : const Iterable.empty(), ..._behaviour is RacingLinesBehaviour
            ? _buildLinesSettings()
            : const Iterable.empty(), ..._behaviour is BubblesBehaviour
            ? _buildBubblesSettings()
            : const Iterable.empty(), ..._behaviour is SpaceBehaviour
            ? _buildSpaceSettings()
  ...
  List<Widget> _buildSpaceSettings() {
    return <Widget>[
      Row(
        children: <Widget>[
          ElevatedButton(
            child: const Text('啟動示例飛濺'),
            onPressed: () {
              setState(() {
                Navigator.of(context).push(AnimatedSimpleFadeRoute(
                  child: const AnimatedSpaceSplash(),
                ));
              });
            },
          ),
