...
class ListSlidableWidget4 extends StatelessWidget {
  const ListSlidableWidget4({Key? key,}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SliverFixedExtentList(
      ...
      delegate: SliverChildBuilderDelegate(
          ...
          //创建列表项
          return ListItem(title: 'Slash', subtitle: '${index + 1}', color: Colors.green[100 * (index % 10)],);
        },
        childCount: 10,
      ),
    );
...
class ListItem extends StatefulWidget {
  const ListItem({Key? key, required this.title, required this.subtitle, required this.color}) : super(key: key);
