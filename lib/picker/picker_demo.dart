...
import 'package:flutter_practice2/picker/picker_data.dart';
import 'package:flutter_practice2/picker/picker_test.dart';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        theme: ThemeData(
          ...
          primaryTextTheme: const TextTheme().apply(fontFamily: _fontFamily),
        ),
        scrollBehavior: const MaterialScrollBehavior().copyWith(scrollbars: true, dragDevices: _kTouchLikeDeviceTypes),
        localizationsDelegates: const [ // 如果要使用本地化，请添加此行，则可以显示中文按钮
        ...
        home: const PickerContent());
  }
}

class PickerContent extends StatefulWidget {
  const PickerContent({Key? key}) : super(key: key);

  @override
  _PickerContentState createState() => _PickerContentState();
}

class _PickerContentState extends State<PickerContent> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (v) => const RawPickerTest())),
      ...
      body: Container(
        ...
        child: ListView(
          children: <Widget>[
            ListTile(
              title: const Text('1. 選擇器秀'),
              onTap: () {
                showPicker(context);
              },
            ),
            ListTile(
              title: const Text('2. 選擇器顯示模式'),
              onTap: () {
                showPickerModal(context);
              },
            ),
            ListTile(
              title: const Text('3. 選擇器顯示圖標'),
              onTap: () {
                showPickerIcons(context);
              },
            ),
            ListTile(
              title: const Text('4. 選擇器顯示（陣列)'),
              onTap: () {
                showPickerArray(context);
              },
            ),
            ListTile(
              title: const Text('5. 選擇器顯示編號'),
              onTap: () {
                showPickerNumber(context);
              },
            ),
            ListTile(
              title: const Text('6. 選擇器顯示數字格式值'),
              onTap: () {
                showPickerNumberFormatValue(context);
              },
            ),
            ListTile(
              title: const Text('7. 選擇器展示日期'),
              onTap: () {
                showPickerDate(context);
              },
            ),
            ListTile(
              title: const Text('8. 選擇器顯示日期時間'),
              onTap: () {
                showPickerDateTime(context);
              },
            ),
            ListTile(
              title: const Text('9. 選擇器顯示日期（自定義)'),
              onTap: () {
                showPickerDateCustom(context);
              },
            ),
            ListTile(
              title: const Text('10. 選擇器顯示日期時間 (24)'),
              onTap: () {
                showPickerDateTime24(context);
              },
            ),
            ListTile(
              title: const Text('11. 選擇器顯示日期時間（圓形背景)'),
              onTap: () {
                showPickerDateTimeRoundBg(context);
              },
            ),
            ListTile(
              title: const Text('12. 選擇器顯示日期範圍'),
              onTap: () {
                showPickerDateRange(context);
              },
            ),
            ListTile(
              title: const Text('13. 持續時間選擇器（時間)'),
              onTap: () {
                showPickerDurationSelect(context);
              },
            ),
            ListTile(
              title: const Text('14. 自定義UI效果（時間)'),
              onTap: () {
                showPickerCustomizeUI(context);
              },
            ),
            ListTile(
              title: const Text('15. 使用onBuilderItem'),
              onTap: () {
  ...
  showPicker(BuildContext context) async {
    Picker picker = Picker(
        ...
        onConfirm: (Picker picker, List value) {
          ...
          LogUtil.e(picker.getSelectedValues());
        });
  ...
  showPickerModal(BuildContext context) async {
    final result = await Picker(
        ...
        onConfirm: (picker, value) {
          LogUtil.e(value.toString());
          LogUtil.e(picker.adapter.text);
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => Scaffold(
                        appBar: AppBar(title: const Text("哈摟")),
                        body: Center(
                            child: Column(
    ...
    LogUtil.e("結果: $result"); // ffoldKey.currentState);
  }
  ...
  showPickerDate(BuildContext context) {
    Picker(
        ...
        selectedTextStyle: const TextStyle(color: Colors.blue),
        onConfirm: (Picker picker, List value) {
          LogUtil.e((picker.adapter as DateTimePickerAdapter).value);
        }).showDialog(context);
  }
  ...
  showPickerDateRange(BuildContext context) {
    LogUtil.e("取消文字: ${PickerLocalizations.of(context).cancelText}");
  ...
  showPickerCustomizeUI(BuildContext context) {
    ...
    Picker(
        ...
        title: const Padding(
          ...
          child: Text("選擇時間", style: txtStyle),
        ),
        onConfirm: (Picker picker, List value) {
          LogUtil.e(picker.adapter.text);
  ...
  showPickerDurationSelect(BuildContext context) {
    ...
    final p1 = Picker(
        ...
        onConfirm: (picker, selected) {
          showMsg(
              "開始: ${range[0].toString()}, 結束: ${range[1].toString()}, ${picker.adapter}");
        });
    final p2 = Picker(
        adapter: DateTimePickerAdapter(
          customColumnType: [6, 7, 4],
