...
import 'package:flutter_practice2/livekit/theme.dart';
...
abstract class ParticipantWidget extends StatefulWidget {
  ...
  static ParticipantWidget widgetFor(ParticipantTrack participantTrack) {
    ...
    throw UnimplementedError('未知參與者類型');
  }
  ...
  const ParticipantWidget({this.quality = VideoQuality.MEDIUM, Key? key,}) : super(key: key);
}
...
class LocalParticipantWidget extends ParticipantWidget {
  ...
  const LocalParticipantWidget(
      this.participant,
      this.videoTrack,
      this.isScreenShare, {Key? key,}) : super(key: key);
...
abstract class _ParticipantWidgetState<T extends ParticipantWidget> extends State<T> {
  ...
  // 通知 Flutter 需要重新構建 UI，但我們這裡沒有設置任何內容
  // 因為更新的值是計算屬性。
...
class _RemoteParticipantWidgetState
  ...
  List<Widget> extraWidgets(bool isScreenShare) => [
    Row(
      ...
      children: [
        // RemoteTrackPublication<RemoteVideoTrack> 的菜單
        if (videoPublication != null)
          RemoteTrackPublicationMenuWidget(
            pub: videoPublication!,
            icon: isScreenShare ? EvaIcons.monitor : EvaIcons.video,
          ),
        // RemoteTrackPublication<RemoteAudioTrack> 的菜單
