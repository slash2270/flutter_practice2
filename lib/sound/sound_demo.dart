...
// If someone update the following comment, please update also the Examples/README.md file and the code inside Examples/lib/demo/redux_github_search_widget.dart
/*
...
class SoundDemo extends StatefulWidget {
  const SoundDemo({Key? key}) : super(key: key);
...
class _SoundDemoState extends State<SoundDemo> {
  ...
  Future<Uint8List?> makeBuffer(String path) async {
    try {
      ...
      playerModule.logger.i('該文件的長度為 ${contents.length} 個字節。');
      return contents;
...
*/
  ...
  void pauseResumeRecorder() async {
    try {
      ...
      recorderModule.logger.e('錯誤: $err');
    }
    setState(() {});
  }
  ...
  AssetImage recorderAssetImage() {
    if (onStartRecorderPressed() == null) {
      return const AssetImage('assets/ic_mic_disabled.png');
    }
    return (recorderModule.isStopped)
        ? const AssetImage('assets/ic_mic.png')
        : const AssetImage('assets/ic_stop.png');
  }
